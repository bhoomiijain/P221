    ...
    <div class="min-h-screen flex" style="background: #001E2B; font-family: 'Inter', sans-serif;">
        ...
        <div class="flex-1 flex items-center justify-center px-6 py-12 relative" style="background: #F9FBFA;">
            ...
            <div class="relative w-full max-w-md">
                ...
                <div class="mt-6 p-4 rounded-xl" style="background: #f0fdf9; border: 1.5px solid #99f6e4;">
                    <p class="text-xs font-bold mb-3" style="color:#064e3b;">Demo accounts (password: <code>password</code>)</p>
                    <div class="space-y-2">
                        <button v-for="acc in page.props.demoAccounts" :key="acc.email" type="button"
                            class="w-full text-left px-3 py-2 rounded-lg text-xs font-semibold hover:bg-white transition-colors"
                            style="color:#0f766e; border: 1px solid #ccfbf1;"
                            @click="fillDemo(acc)">
                            {{ acc.role }} — {{ acc.email }}
                        </button>
                    </div>
                    <p class="text-[10px] mt-2" style="color:#64748b;">Click an account, then Sign In. Run <code>php artisan db:seed</code> if login fails.</p>
                </div>
